...
IX. Cross-Device Synchronization: By default, all edits, styling updates, and structural changes must be fully responsive and applied to all device tiers (Mobile, Tablet, Desktop). The agent must ensure that a feature works and looks premium on all screen sizes unless a specific device tier is explicitly mentioned in the prompt. If a specific device is targeted (e.g., "for mobile only"), the agent must isolate those changes using media queries or responsive classes without affecting other breakpoints.
X. Mobile-First Design Philosophy: When implementing responsive designs, always prioritize the mobile viewport first. Start with base styles for mobile devices, and then use `min-width` media queries to layer on tablet and desktop enhancements. This ensures that the mobile experience is optimized and intentional, rather than being an afterthought or a scaled-down desktop view.
